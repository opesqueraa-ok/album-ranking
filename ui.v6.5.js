// UI v6.5 wiring (lang/export/import/clear + notes + clear scores + better export name) — v6.5.1 build
(function () {
  ...
  const I18N = {
    en: {
      ...
      clearScores: "Clear Score", import: "Import Data", export: "Export Data", total: "Duration", avg: "Average",
      confirmClear: "Are you sure you want to clear all fields?", notePrompt: "Write a note for this track:",
      imported: "Imported.", invalidFile: "Invalid file.", exportFail: "Could not export."
    },
    es: {
      ...
      clearScores: "Limpiar puntajes", import: "Importar datos", export: "Exportar datos", total: "Duración total", avg: "Promedio",
      confirmClear: "¿Seguro que deseas borrar todos los campos?", notePrompt: "Escribe una nota para esta canción:",
      imported: "Importado.", invalidFile: "Archivo inválido.", exportFail: "No se pudo exportar."
    }
  };
  ...
  function applyI18N(lang){
    ...
    [["applyCount","apply"],["addRow","addRow"],["delRow","delRow"],["clearAll","clearAll"],["importLabel","import"],["exportJSON","export"],["clearScores","clearScores"]]
  ...
  function currentLang(){ return (localStorage.getItem(KEY_LANG) || ((navigator.language||"en").startsWith("es")?"es":"en")); }
  ...
  function exportJSON(){
    try{
      ...
      const album=safeName(s.album||"Album"); const artist=safeName(s.artist||"");
    ...
    r.onload=e=>{
      try{
        ...
        if(obj && obj.notes){
          ...
          renderNotesOutput(); const final=$("#finalNotes"); if(final) final.value=obj.notes.final||"";
        }
  ...
  // Notas por canción (botón ✏️ a la derecha)
  function ensureNoteButtons(){
    ...
    [...container.children].forEach((row, idx)=>{
      ...
      btn.addEventListener("click", ()=>{
        ...
        const notes=getNotesForCurrent(); const prev=notes.trackNotes[idx]||"";
        const text=prompt(t.notePrompt, prev==null? "": String(prev));
        if(text==null) return;
        notes.trackNotes[idx]=text.trim(); setNotesForCurrent(notes);
        renderNotesOutput(); btn.classList.toggle("active", !!notes.trackNotes[idx]);
      });

      const notes=getNotesForCurrent(); btn.classList.toggle("active", !!notes.trackNotes[idx]);
    });
  }
  function renderNotesOutput(){
    const out=$("#notesOutput"); if(!out) return;
    const s=window.AlbumApp.getState(); const notes=getNotesForCurrent();
    const lines=[]; const tracks=s.tracks||[];
    tracks.forEach((t,i)=>{ const note=notes.trackNotes[i]; if(note && note.trim()){ const name=t?.name||`Track ${i+1}`; lines.push(`${i+1}. ${name}: ${note.trim()}`); }});
  ...
  function bindFinalNotes(){
    const ta=$("#finalNotes"); if(!ta || ta._bound) return; ta._bound=true;
    ta.addEventListener("input", ()=>{ const n=getNotesForCurrent(); n.final=ta.value||""; setNotesForCurrent(n); renderNotesOutput(); });
  }
  function observeTrackList(){
    const target=document.getElementById("tracks"); if(!target || target._observerAttached) return; target._observerAttached=true;
  ...
  // Bind UI
  function bindUI(){
    let LANG=localStorage.getItem(KEY_LANG) || ((navigator.language||"en").startsWith("es")?"es":"en");
    applyI18N(LANG);
    ...
    window.addEventListener("album-autofilled", ()=>{ bindFinalNotes(); observeTrackList(); const notes=getNotesForCurrent(); const ta=$("#finalNotes"); if(ta) ta.value=notes.final||""; renderNotesOutput(); });
  }
