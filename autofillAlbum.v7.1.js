/*! Album Autofill v7.1 — MusicBrainz + iTunes, iOS-safe */
(() => {
  ...
  function rankPicker(initial) {
    ...
    function setFromNumber(v) {
      ...
      const dec = Math.round((v - b) * 10) / 10;
      iSel.value = String(b);
      dSel.disabled = false;
      fillDec(b < 10);
    ...
    function current() {
      const ib = iSel.value;
      if (ib === '') return NaN;
      const dd = parseFloat(dSel.value || '0');
      return parseFloat(ib) + dd;
    }
    function trigger() {
  ...
  function makeRow(i, data = {}) {
    ...
    // ENSURE there's a placeholder cell for the notes button (ui.* injects it later)
    const notesCell = document.createElement('div');

    row.append(n, dur, name, picker.el, pill, notesCell);
  ...
  // expone para que ui.v7.1.js pueda reconstruir filas tras ordenar/restaurar
  ...
  // --- Estado ---
  const KEY = 'albumrater_v7_1_state';
  function getState() {
    const el = tracksEl();
    ...
    (s.tracks || []).forEach((t, i) =>
      el.appendChild(
        makeRow(i, {
          ...
          dur: t.dur,
          name: t.name,
          score: typeof t.score === 'number' ? t.score : NaN,
        })
      )
    );
  ...
  function load() {
    try {
      // migración desde v7_0 o v6 si había algo
      const rawNew = localStorage.getItem(KEY);
      const rawV7 = localStorage.getItem('albumrater_v7_state');
      const rawV6 = localStorage.getItem('albumrater_v6_state');
      if (rawNew) {
        setState(JSON.parse(rawNew));
        return;
      }
      if (rawV7) {
        setState(JSON.parse(rawV7));
        save();
        return;
      }
      if (rawV6) {
        setState(JSON.parse(rawV6));
        save();
        return;
      }
    } catch {}
  ...
  function render() {
    ...
    thead.innerHTML =
      ...
      '</th><th style="width:36px">#</th><th>' +
      (LANG === 'es' ? 'Nombre' : 'Name') +
      '</th><th style="width:90px">Score</th></tr>';
  ...
  function drawChart(id, values) {
    ...
    const P = { l: 60, r: 20, t: 20, b: 36 },
      W = canvas.width - P.l - P.r,
      H = canvas.height - P.t - P.b;
  ...
  async function fetchReleaseMB(id) {
    ...
    const res = await safeFetch(url);
    if (!res.ok) throw new Error('MB release failed');
    const data = await res.json();
    const tracks = [];
    (data.media || []).forEach((m) => (m.tracks || []).forEach((t) => tracks.push({ title: t.title, duration: t.length ? mmss(t.length) : '' })));
    ...
    return {
      ...
      tracks,
      totalTime: totalMs ? mmss(totalMs) : '',
      id: data.id,
    };
  }
  ...
  function ensureModal() {
    ...
    modal.style.cssText =
      'position: fixed; inset: 0; display: none; align-items: center; justify-content: center; background: rgba(0,0,0,.55); z-index: 9999; font-family: system-ui,-apple-system,Segoe UI,Roboto,Arial;';
  ...
  function showCandidates(cands) {
    ...
    list.innerHTML = '';
    if (!cands.length) {
      list.innerHTML = `<div style="padding:18px">No se encontraron ediciones.</div>`;
    } else {
      cands.forEach((c) => {
        const row = document.createElement('div');
        ...
        row.innerHTML = `<div style="min-width:0">
            ...
            <div style="opacity:.8;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${c.artistCredit || ''} ${
          c.date ? '· ' + c.date : ''
        } ${c.country ? '· ' + c.country : ''}</div>
          </div>
  ...
  async function runAutofill() {
    ...
    const album = $('#album').value.trim();
    if (!artist || !album) {
      alert(LANG === 'es' ? 'Escribe artista y álbum primero.' : 'Type artist and album first.');
      return;
    }
    try {
    ...
    try {
      ...
      if (!res.ok) throw new Error('itunes search');
      const json = await res.json();
      if (!json.resultCount) throw new Error('no results');
      const low = (s) => (s || '').toLowerCase();
      const best =
        json.results.find((r) => low(r.collectionName).includes(low(album)) && low(r.artistName).includes(low(artist))) || json.results[0];
      ...
      if (!res2.ok) throw new Error('itunes lookup');
      const json2 = await res2.json();
      if (!json2.results || json2.results.length <= 1) throw new Error('no tracks');
  ...
  function bindCore() {
    ...
    if (b && !b._bound) {
      b.addEventListener('click', runAutofill);
      b._bound = true;
    }
    const add = document.getElementById('addRow');
    if (add && !add._bound) {
      add._bound = true;
      add.addEventListener('click', () => ensureRows(document.getElementById('tracks').children.length + 1));
    }
    const del = document.getElementById('delRow');
    if (del && !del._bound) {
      del._bound = true;
      del.addEventListener('click', () => ensureRows(Math.max(1, document.getElementById('tracks').children.length - 1)));
    }
    const app = document.getElementById('applyCount');
    if (app && !app._bound) {
      app._bound = true;
      app.addEventListener('click', () => ensureRows(parseInt(document.getElementById('trackcount').value || '1', 10)));
    }
    const cov = document.getElementById('cover');
    if (cov && !cov._bound) {
      cov._bound = true;
      cov.addEventListener('change', (ev) => {
        const f = ev.target.files[0];
        if (!f) return;
        const r = new FileReader();
        r.onload = (e) => {
          document.getElementById('coverOut').src = e.target.result;
          save();
        };
        r.readAsDataURL(f);
      });
    ...
    if (sortBtn && !sortBtn._bound) {
      ...
      sortBtn.addEventListener('click', () => {
        ...
        if (!SORT_STATE.active) {
          ...
          setSortButton(true);
          render();
          // Notificar al UI para que reinyecte botones de nota si hace falta
          window.dispatchEvent(new CustomEvent('album-autofilled', { detail: { tracks: merged.map((t) => ({ title: t.name, duration: t.dur })) } }));
          return;
        }
        ...
        setSortButton(false);
        render();
        window.dispatchEvent(new CustomEvent('album-autofilled', { detail: {} }));
      });
