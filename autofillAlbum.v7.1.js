/*! Album Autofill v7.1 — MB + iTunes, iOS-safe */
(() => {
  ...
  function setSortButton(active){ const btn=document.getElementById('sortTop10'); if(!btn) return; btn.textContent = active ? 'Restore album order' : 'Sort Top 10'; }
  ...
  function rankPicker(initial){
    ...
    const iSel=document.createElement('select'); let o=document.createElement('option'); o.value=''; o.textContent='-'; iSel.appendChild(o);
  ...
  function makeRow(i,data={}){
    ...
    function paint(v){ if(!Number.isFinite(v)){ pill.style.background=NEUTRAL; pill.textContent='-'; return; } const c=colorFor(v); pill.style.background=c; pill.textContent=v.toFixed(1).replace(/\.0$/,''); }
  ...
  function load(){
    try{
      const rawOld = localStorage.getItem('albumrater_v6_state');
      const rawNew = localStorage.getItem(KEY);
      if(rawNew){ setState(JSON.parse(rawNew)); return; }
      if(rawOld){ setState(JSON.parse(rawOld)); save(); return; }
    }catch(e){}
    ensureRows(7); render();
  }
  ...
  function render(){
    ...
    const el = tracksEl();
    const tracks=[...el.children].map(r=>r.value()).filter(tr=>tr.name||tr.dur||Number.isFinite(tr.score)).sort((a,b)=>a.n-b.n);

    const table=document.getElementById('table'); table.innerHTML='';
    const thead=document.createElement('thead'); thead.innerHTML='<tr><th style="width:80px">'+(LANG==='es'?'Duración':'Duration')+'</th><th style="width:36px">#</th><th>'+(LANG==='es'?'Nombre':'Name')+'</th><th style="width:90px">Score</th></tr>'; table.appendChild(thead);
  ...
  async function searchReleasesMB(artist, album) {
    ...
    const res = await safeFetch(url); if (!res.ok) throw new Error('MB search failed');
  ...
  async function fetchCoverMB(releaseId){
    const meta = await safeFetch(`https://coverartarchive.org/release/${releaseId}`); if (!meta.ok) return null; const json = await meta.json().catch(()=>null);
    const big = json?.images?.find(i => i.thumbnails?.large) || json?.images?.[0]; return big?.image || big?.thumbnails?.large || null;
  }

  function ensureModal() {
    let modal = document.getElementById('albumAutofillModal'); if (modal) return modal;
    modal = document.createElement('div'); modal.id = 'albumAutofillModal';
  ...
  async function runAutofill(){
        ...
        modal.querySelectorAll('.aam_pick').forEach(btn => {
          btn.addEventListener('click', async (ev) => {
            ...
            try {
              ...
              console.error(e); alert('No se pudo cargar la edición seleccionada.');
    ...
    try{
      ...
      fillDOM(payload); window.dispatchEvent(new CustomEvent('album-autofilled', { detail: payload })); return;
    }catch(e){ console.warn('iTunes error', e); }
    alert('No encontré info automática. Puedes llenar manualmente.');
  }

  function fillDOM(p){ if(p.title) $('#album').value=p.title; if(p.artist) $('#artist').value=p.artist; if(p.year) $('#released').value=p.year; if(p.trackCount) $('#trackcount').value=String(p.trackCount); if(p.coverUrl) $('#coverOut').src=p.coverUrl; }

  function bindCore(){
    const b = document.getElementById('btnBuscarAlbum'); if(b && !b._bound){ b.addEventListener('click', runAutofill); b._bound = true; }
    ...
    if (sortBtn && !sortBtn._bound) {
      ...
      sortBtn.addEventListener('click', () => {
        ...
        if (SORT_STATE.snapshot) {
          const original = SORT_STATE.snapshot.map(x => ({ ...x })); el.innerHTML = '';
  ...
  window.addEventListener('album-autofilled', (e) => {
    ...
    [...document.getElementById('tracks').children].forEach((row, i) => {
      const inputs = row.querySelectorAll('input'); const t = list[i] || {};
      if (inputs[1]) inputs[1].value = t.duration || ''; if (inputs[2]) inputs[2].value = t.title || '';
    });
  ...
  function boot(){ if(document.readyState==='complete' || document.readyState==='interactive'){ try{ bindCore(); load(); }catch(e){ console.error(e); } } else { document.addEventListener('DOMContentLoaded', ()=>{ try{ bindCore(); load(); }catch(e){ console.error(e); } }); } }
  boot();
})();
