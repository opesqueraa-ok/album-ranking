...
(() => {
  ...
  async function itunesFetchTracks(collectionId){
    ...
    const tracks = json.results.slice(1).filter(x=>x.wrapperType==='track').map(t => ({
      n: t.trackNumber, title: t.trackName, duration: t.trackTimeMillis ? mmss(t.trackTimeMillis) : ''
    }));
  ...
  async function runAutofill(){
    ...
    const album  = $(SELECTORS.album).value.trim();
